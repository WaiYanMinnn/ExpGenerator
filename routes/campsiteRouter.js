...
.get((req,res,next)=>{
    ...
    .then(campsite=>{
        if(campsite){
            ...
            err = new Error(`Campsite ${req.params.campsiteId} is not found`);
            err.status=404;
            return next(err);
        }
    })
...
.put((req,res)=>{
    res.statusCode=403;
    res.end(`Put opertion is not support on /campsites/${req.params.campsiteId}/comments`);
})
.delete((req,res,next)=>{
    Campsite.findById(req.params.campsiteId)
    .then(campsite=>{
        if(campsite){
            for(i= campsite.comments.length-1; i>=0; i--){
